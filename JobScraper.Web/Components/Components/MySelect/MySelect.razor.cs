...
public partial class MySelect<TItem> : ComponentBase
{
    ...
    private bool DefaultFilterPredicate(TItem item, string filterString) =>
        StringSelector(item).Contains(filterString, StringComparison.OrdinalIgnoreCase);
    ...
    {
        if (_prevCanFilter == CanFilter && _prevFilterPredicate == FilterPredicate && _prevFilterText == _filterText &&
