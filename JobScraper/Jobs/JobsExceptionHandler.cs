...
public class JobsExceptionHandler : ITickerExceptionHandler
{
    ...
    public JobsExceptionHandler(ILogger<JobsExceptionHandler> logger)
    {
        _logger = logger;
    ...
    {
        _logger.LogError(exception, "An error occurred while processing the job. Ticker ID: {TickerId}, Type: {TickerType}",
            tickerId, tickerType);
